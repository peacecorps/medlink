...
RSpec.describe TwilioController do
  ...
  context "with a valid sid" do
    When(:result) { post :receive, params: { AccountSid: twilio.sid, From: phone.number, To: twilio.number, Body: "help" } }
  ...
  context "with a new phone number" do
    ...
    When(:result) { post :receive, params: { AccountSid: twilio.sid, From: number, To: twilio.number, Body: "help" } }
  ...
  context "with an invalid sid" do
    When(:result) { post :receive, params: { AccountSid: "1234", From: phone.number, To: twilio.number, Body: "help" } }

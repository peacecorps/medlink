...
RSpec.describe OrderResponsePresenter do
  ...
  context "old order" do
    Given(:order) { build :order, response: response, created_at: 2.years.ago, delivery_method: :denial }

    When(:result) { OrderResponsePresenter.new order }

    Then { result.how_past_due.match(/2 years/)                       }
  ...
  context "new order" do
    Given(:order) { build :order, response: response, created_at: 1.day.ago, delivery_method: :delivery, duplicated_at: 2.hours.ago }
